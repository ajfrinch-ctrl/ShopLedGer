 ...
 *
 *   node scripts/write-atomic.mjs .cache/og.jpg.tmp public/og.jpg
 *
 ...
 * `rename` is injectable because EXDEV — the refusal the "stage under the
 * workspace `.cache/` directory" contract rests on — cannot be provoked portably.
 */
    ...
    if (err?.code === "EXDEV") {
      throw new Error(
        `${staged} is on another filesystem than ${target}, so the hand-over cannot be a `
          + "rename — stage under the workspace .cache/ directory instead",
      );
    }
    throw err;
  }
